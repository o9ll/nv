...
import { BRAND_NAME, BRAND_SUPPORT_URL } from "@shared/branding";
...
function formatCommitLink(hash?: string | null, shortHash?: string | null) {
    if (!hash || !shortHash) return null;
    return repoUrl ? `[${shortHash}](<${repoUrl}/commit/${hash}>)` : shortHash;
}
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • ${clientInfo.shortHash}` : ""}`;
    ...
    const buildDetails = [`v${VERSION}`];
    const commitLink = formatCommitLink(gitHash, gitHashShort);
    if (commitLink) buildDetails.push(commitLink);
    if (!IS_EQUIBOP) {
        buildDetails.push(`${SettingsPlugin.getVersionInfo()} - ${Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(BUILD_TIMESTAMP)}`);
    }

    const info = {
        [BRAND_NAME]: buildDetails.join(" • "),
    ...
    const commonIssues = {
        ...
        [`${BRAND_NAME} DevBuild`]: !IS_STANDALONE,
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Paragraph>You are using an outdated version of {BRAND_NAME}. Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of {BRAND_NAME}, which we do not provide support for.</Paragraph>

                        <Paragraph className={Margins.top8}>
                            {repoUrl
                                ? <>We only provide support for <Link href={repoUrl}>official builds</Link>. Please switch to an official build before asking for support.</>
                                : <>We only provide support for official builds. Please switch to an official build before asking for support.</>
                            }
                        </Paragraph>
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message {BRAND_NAME} or Vencord plugin developers for support!
                <br />
