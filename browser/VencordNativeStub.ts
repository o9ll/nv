...
window.VencordNative = {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: repoUrl }),
    ...
    quickCss: {
        get: async () => DataStore.get(QUICKCSS_KEY).then(s => s ?? DataStore.get(LEGACY_QUICKCSS_KEY).then(legacy => legacy ?? "")),
        set: async (css: string) => {
            await DataStore.set(QUICKCSS_KEY, css);
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem(SETTINGS_KEY) || localStorage.getItem(LEGACY_SETTINGS_KEY) || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem(SETTINGS_KEY, JSON.stringify(s)),
